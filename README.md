...
## Trees
A `Tree`, generically speaking, is a (potentially nested) collection.

A traversal function `n -> Tree|Falsy`, given a tree node `n`, generates either the next `Tree` to traverse or a falsy value `Falsy` to indicate termination.

All tree functions below take a `traverse` function, allowing for traversal of subsets of the tree. The default traverse function is the trivial one: it generates the unfiltered collection at the given node.

*Note*: Be careful about cyclic structures that can result in infinite loops, such as objects with references to themselves. There are cases where you'd intentionally want to visit the same node multiple times, such as traversing a directed acyclic graph (which would work just fine and eventually terminate, but would visit a node once for each parent it has connected to it) - but it's up to the user to be sure you don't create infinite loops.

### isTraversable
`x -> Boolean`
`true` if value `x` is a collection.

### traverse
`x -> x|false`
The trivial traversal function. It returns `x` if `isTraversable(x)` and `false` otherwise.

### walk
`traverse -> (pre, post=_.noop) -> tree -> x`
A depth-first search which visits every node returned by `traverse` recursively. Both pre-order and post-order traversals are supported (and can be mixed freely). `walk` supports exiting iteration early by returning a truthy value from either `pre` or `post`. The returned value is that of the last `post` call.

The `pre`, `post`, and `traverse` functions take the same arguments in the following order:

1. `node`: the current node.
2. `index`: the `node`'s index in its containing collection (generated by applying `traverse` on its parent).
3. `parents`: path `[n0, ..., nr]` to the root node, where `n0` is `node`'s direct parent and `nr` is the root.
4. `parentIndexes`: is to `parents` as `index` is to `node`.
...
Tree version of `_.reduce`. Pre-order `walk` the `tree`, using `accumulator` as the pre-order function. `accumulator`'s signature is `acc -> ...pre_args -> x`, where `acc` is the accumulated value and `pre_args` are the same arguments as `walk`'s `pre` function.

### treeToArray
`traverse -> tree -> [n0, ..., nn]`
Flattens the tree nodes into an array of nodes, simply recording the node values in pre-order traversal.

### treeToArrayBy
`traverse -> f -> tree -> [f(n0), ..., f(nn)]`
Basically `_.flow(treeToArray, _.map(f))`
