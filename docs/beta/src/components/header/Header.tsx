...
const toolTipInOut = { heading: `What was that method? Let's find it!` , 
...
const toolTipName = { heading: `Looking for a specific method?`, 
  content: `Search for methods by name or description.`}


export let PageHeader = ({ search, input, output, page, dispatch, responsive }) => (
  <>
    <Box>
    ...
    <Flex direction={'row'} gap={1} display={responsive.nav.showDesktopInputs}>
      <SearchInputs input={input} output={output} search={search} 
                    toolTipName={toolTipName} toolTipInOut={toolTipInOut} 
                    dispatch={dispatch} responsive={responsive} />
    </Flex>
    <Spacer/>
    <Box>
      <Menu>
