...
describe('Regexp Functions', () => {
  ...
  it('matchAnyWord', () => {
    console.time ('matchAnyWord')
    const reText = 'Some text'
    const text = 'Here is some text to test'
    const match = f.matchAnyWord(reText)
    expect(match(text)).to.equal(true)
    console.timeEnd ('matchAnyWord')
  })
   it('matchAnyWord1', () => {
     console.time ('matchAnyWord__punctuation');
    const reText = 'Some text'
    const text = 'Here... is, some; Text to -- test.'
    let match = f.matchAnyWord(reText)

    expect(match(text)).to.equal(true)
    console.timeEnd('matchAnyWord__punctuation')
   })
it('matchAnyWord2', () => {
     console.time ('matchAnyWord__long_body')
    let reText1 = 'Some text'
    let text1 = moby
    let match1 = f.matchAnyWord(reText1)

    expect(match1(text1)).to.equal(true)
    console.timeEnd('matchAnyWord__long_body')
  })
it('matchAnyWord3', () => {
     console.time ('matchAnyWord__long_query')
    const reText = 'Ishmael some text'
    const text = moby
    const match = f.matchAnyWord(reText);

    expect(match(text)).to.equal(true)
    console.timeEnd('matchAnyWord__long_query')
  })
  it('matchAllWords', () => {
    console.time ('matchAllWords')
    const reText = 'Some text'
    const text = 'Here is some to test'
    const match = f.matchAllWords(reText)
    expect(match(text)).to.equal(false)
    console.timeEnd ('matchAllWords')
  })
  it('matchAllWords1', () => {
    console.time ('matchAllWords__capitalize_Punctuate')
    const reText = 'It has all the... Things..'
    const text = 'it has, ALL THE THINGS!!!'
    const match = f.matchAllWords(reText)

    expect(match(text)).to.equal(true)
    console.timeEnd ('matchAllWords__capitalize_Punctuate')
  })
  it('matchAllWords2', () => {
    console.time ('matchAllWords__mix_order')
    const reText = 'It has all the... Things..'
    const text = 'ALL THE THINGS **it** HAS!!!'
    const match = f.matchAllWords(reText)

    expect(match(text)).to.equal(true)
    console.timeEnd ('matchAllWords__mix_order')
  })
  it('matchAllWords3', () => {
    console.time ('matchAllWords__long_match')
    const reText = moby
    const text = moby
    const match = f.matchAllWords(reText)

    expect(match(text)).to.equal(true)
    console.timeEnd ('matchAllWords__long_match')
  })
  it('matchAllWords5', () => {
    console.time ('matchAllWords__long_match_backwards_fail')
    const reText = moby
    const text = _.reverse(moby).join('')
    const match = f.matchAllWords(reText)

    expect(match(text)).to.equal(false)
    console.timeEnd ('matchAllWords__long_match_backwards_fail')
  })
  it('matchAllWords4', () => {
    console.time ('matchAllWords__long_match_backwards')
    const reText = moby
    const text = _.reverse(_.words(moby)).join(' ')
    const match = f.matchAllWords(reText)
    expect(match(text)).to.equal(true)
    console.timeEnd ('matchAllWords__long_match_backwards')
  })
})
